...
public:
	...
	TuioObject(long _sid, int _fid, float _xpos, float _ypos, float _angle) : TuioContainer( _sid,  _xpos,  _ypos){
        fiducialId = _fid;
	    angle = _angle;
		rotation_speed = 0.0f;
		rotation_accel = 0.0f;
	...
	TuioObject(TuioObject * _tuioObject) : TuioContainer(_tuioObject){
	...
	void update (float _xpos, float _ypos, float _angle, float _xspeed, float _yspeed, float _rspeed, float _maccel, float _raccel) {
		TuioContainer::update(_xpos,_ypos,_xspeed,_yspeed,_maccel);
		angle = _angle;
		rotation_speed = _rspeed;
		rotation_accel = _raccel;
	};
	
	void update (TuioObject * _tuioObject) {
		TuioContainer::update(_tuioObject);
		angle = _tuioObject->getAngle();
		rotation_speed = _tuioObject->getRotationSpeed();
		rotation_accel = _tuioObject->getRotationAccel();
	};
