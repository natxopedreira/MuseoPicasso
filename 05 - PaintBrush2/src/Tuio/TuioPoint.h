...
public:
	...
	TuioPoint(float _xpos, float _ypos){
		xpos = _xpos;
		ypos = _ypos;
	};
	
	TuioPoint(TuioPoint * _tuioPoint){
	...
	void update (TuioPoint * _tuioPoint) {
	    xpos = _tuioPoint->getX();
		ypos = _tuioPoint->getY();
	};
	...
	float getDistance(TuioPoint * _tuioPoint) {
		float dx = xpos-_tuioPoint->getX();
		float dy = ypos-_tuioPoint->getY();
		return sqrtf(dx*dx+dy*dy);
	};
	
	float getAngle(TuioPoint * _tuioPoint) {
	...
	float getAngleDegrees(TuioPoint * _tuioPoint) {
