...
void testApp::setup(){
    ...
    #ifdef IPVIDEO
        videoIp.setURI("http://192.168.0.90/axis-cgi/mjpg/video.cgi?resolution=640x480");
        videoIp.setUsername("root");
        videoIp.setPassword("kalandraka");
        videoIp.connect();
    #else
    ...
    photoViewer.loadMovie("pasafotos_flip.mov");
    photoViewer.setLoopState(OF_LOOP_NORMAL);
    ...
    fboSalida.allocate(1920,1080,GL_RGB);

    fboSalida.begin();
    ofClear(0);
    fboSalida.end();

    viewport.loadSettings(0);
}
...
void testApp::draw(){
    ofBackground(0);
    viewport.draw(fboSalida.getTextureReference());
...
void testApp::updateStateHunter(){
    ...
    #ifdef IPVIDEO
        videoIp.update();
	if (videoIp.isFrameNew()){
	    videoColorCvImage.setFromPixels(videoIp.getPixels(), width, height);
    #else
        video.update();
    if (video.isFrameNew()){
